...
<input
          ...
          <table className="table table-striped table-hover border opacity">
            <tbody id="bad">
              {badList.map((item, i) => (
                <tr key={i}>
                  <td>
                    <input type="checkbox" className="form-check-input" value={item._id} onChange={handleOnSelect}
                    checked ={idsToDelete.includes(item._id)}/>
                  </td>
                  <td>{i + 1}</td>
                  <td>{item.task}</td>
...

      {
        ...
        <button className="btn btn-danger btn-lg" onClick={() => handOnDelete(idsToDelete)}>
          <i className="fa-solid fa-trash"></i> Delete {idsToDelete.length}{""} task
        </button>
      </div>
